...
<div class="d-flex align-items-center ml-3"><!-- Entête de l'item  -->
  ...
  <div>
    ...
    <?php } ?><br>
    Catégorie : <a href="category/<?= $this->clean($category['catid']). '/1/' . $this->clean($category['slug']);?>"><?= $this->clean($category['name']); ?></a> |&nbsp;<a href="<?= !ISSET($_SESSION['id_user']) ? "extendedcard/" . $this->clean($item['itemid'])  . "/1/" .$this->clean($item['slug']). "#comments" : "extendedcard/indexuser/" . $this->clean($item['itemid']). "/1/" .$this->clean($item['slug']). "#comments" ?>">Commentaires (<?= $number_of_comments ;?>)</a><br>
    </div>
  </div>
